...
namespace curves {
    ...
    Vector3d Ellipse::get_point(const double t) const noexcept {
        return Vector3d(std::cos(t) * get_radius_x(), std::sin(t) * get_radius_y(),
            0.);
    }

    Vector3d Ellipse::get_first_derivative(const double t) const noexcept {
        return Vector3d(-std::sin(t) * get_radius_x(), std::cos(t) * get_radius_y(),
            0.);
    }
